---
description: Orchestrate the full DDD feature workflow for React + TypeScript + Vite — spec clarification, domain design, strict TDD implementation, adversarial review. Use when starting any non-trivial feature or when the user asks to "build a feature with DDD"
---

React + TypeScript + Vite プロジェクトで機能開発の**全工程**を規律あるワークフローとして進める。このスキルはオーケストレーターであり、各フェーズの詳細は兄弟スキルに委譲する。
...
│  レイヤー構成・型・集約境界・Repository IF を決める  │
...
| 規約 | 内容 |
|------|------|
| クラス禁止 | ドメイン層・アプリケーション層は `type` + コンパニオンオブジェクト。ブランド型で primitive obsession を防ぐ |
| Repository のみ class 可 | インターフェースはドメイン層、class 実装はインフラ層のみ |
| DIP | 依存は常に内側(ドメイン)へ向く。外側の具象はコンポジションルートで注入 |
| DI | React Context によるコンポジションルート。ユースケースはファクトリ関数 + 依存を引数で受ける |
...
2. `design-domain` を起動 — `DueDate` VO、`Todo` への状態追加(判別 Union)、`ITodoRepository` への影響を設計して提示
3. `tdd-cycle` を起動 — `DueDate.of` のバリデーションテストから開始し、VO → 集約 → ユースケース → hook の順に Red-Green-Refactor
